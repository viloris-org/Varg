...
mod tests {
    ...
    fn problem_report_includes_quick_fix() {
        let report = ProblemReport {
            ...
            affected: vec!["scripts/player.aster".into()],
            ...
            quick_fix: Some(QuickFixAction {
                ...
                description: "Correct the undefined variable reference in player.aster".into(),
                original_task_id: TaskId::from_u128(2),
                requires_user_input: false,
                fix_task_brief: TaskBrief {
                    task_id: TaskId::from_u128(99),
                    objective: "Fix undefined variable in player.aster".into(),
                    non_goals: vec![],
                    allowed_files: vec!["scripts/player.aster".into()],
                    allowed_entities: vec![],
                    allowed_scenes: vec![],
                    allowed_assets: vec![],
                    allowed_operations: vec!["write_script".into()],
                    forbidden_operations: vec![],
                    acceptance_criteria: vec!["player.aster compiles without errors".into()],
                    expected_artifacts: vec![],
                    review_rubric: ReviewRubric {
                        correctness: vec![],
                        scope: vec![],
