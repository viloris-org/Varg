...
use engine_render::{RenderDevice, RenderTargetDesc, RenderWorld};
...
impl WgpuRenderDevice {
    ...
    pub(crate) fn render_world_to_target(
        ...
        let total_instances: u32 = batches.iter().map(|(_, count, _)| *count).sum();
        tracing::debug!(
            target: "engine",
        ...
        let (tw, th) = {
            ...
            (target._desc.width, target._desc.height)
        };

        let frame_res = self.encode_frame_passes(&batches, &csm, tw, th, true, encoder_label);
